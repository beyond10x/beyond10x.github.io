...
test('requires the exact sorted 21-repository Website roster', async () => {
  const fixture = await createFixture({omitLastRepository: true});

  await assert.rejects(verifyWebsiteInputs(...fixture.paths), /exact sorted 21-repository roster/);
});
