...
`aep plan artifact blocked` groups by the blocker and prints the type; `list` and `board` mark the
...
produce one cannot mint a token, and `aep plan artifact explain` says so. That value comes from the
