...
```shell-session
$ aep plan artifact evidence outbound-claim:q3-uptime \
...
$ aep plan artifact move credential-blocker:api-token-scope --to cleared
credential-blocker:api-token-scope moved open -> cleared (revision 2)
```

Nothing was edited out of a file: `aep plan artifact history` still says the blocker was opened, and
...
`aep plan artifact board` prints — and a column `aep plan serve` draws in a browser, where the rungs
...
`new`, `move`, `board`, `lifecycle` and `validate` all understand it. `aep plan artifact validate`
